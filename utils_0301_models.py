...
class client_model(nn.Module):
    def __init__(self, name, args):
        ...
        if self.name in ['cifar10', 'cifar100'] and self.model_name in ["cnn"]:
        ...
        if self.name in ['tinyimagenet'] and self.model_name in ["cnn"]:
            self.conv1 = nn.Conv2d(in_channels=3, out_channels=64 , kernel_size=5)
            self.conv2 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=5)
        ...
        if self.name == 'Resnet18' or self.model_name == "resnet":          
        ...
        if self.model_name == "mobilenetv2":         
            self.model = mobilenetv2(self.n_cls)
    ...
    def forward(self, x):
        ...
        elif self.model_name in ["resnet", "mobilenetv2", "resnet34"]:
            x = self.model(x)

        elif self.name in ['cifar10','cifar100'] and self.model_name in ["cnn"]:
        ...
        elif self.name in ['tinyimagenet']:
            x = self.pool(F.relu(self.conv1(x)))
            x = self.pool(F.relu(self.conv2(x)))
