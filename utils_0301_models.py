...
class client_model(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.name in ['cifar10','cifar100']:
            ...
            if self.norm == "LN":
                x = self.pool(F.relu(self.conv1(self.bn1(x))))
                x = self.pool(F.relu(self.conv2(self.bn2(x))))
                x = x.view(-1, 64*5*5)
                x = F.relu(self.fc1(self.bn3(x)))
                x = F.relu(self.fc2(self.bn4(x)))
                x = self.fc3(x)             
