...
from utils_dataset_tinyimagenet import ImageFolder_custom
...
class DatasetObject:
    ...
    def set_data(self):
        ...
        # if not os.path.exists('%s/%s' %(self.data_path, self.name)) or self.dataset == 'TinyImageNet':
        if not os.path.exists('%s/%s' %(self.data_path, self.name)):
            ...
            if self.dataset == 'CINIC10':
                ...
                dataset_train_global = torchvision.datasets.ImageFolder('cinic_train',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean=cinic_mean,std=cinic_std)]))
                dataset_test_global = torchvision.datasets.ImageFolder('cinic_test',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean=cinic_mean,std=cinic_std)]))

                # print(dataset_train_global.data.shape)
                # print(dataset_train_global.target.shape)
                
                trn_load = torch.utils.data.DataLoader(dataset_train_global, batch_size=len(dataset_train_global), shuffle=True, num_workers=1)
                tst_load = torch.utils.data.DataLoader(dataset_test_global, batch_size=len(dataset_test_global), shuffle=False, num_workers=1)
                self.channels = 1; self.width = 28; self.height = 28; self.n_cls = 10;
            ...
            if self.dataset == 'TinyImageNet':
                ...
                trn_load = torch.utils.data.DataLoader(xray_train_ds, batch_size=len(xray_train_ds), shuffle=True, num_workers=4)
                tst_load = torch.utils.data.DataLoader(xray_test_ds, batch_size=len(xray_test_ds), shuffle=False, num_workers=4)
            ...
            if self.dataset not in ['emnist', "emnist26"]:
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_x, data_y=True, train=False, dataset_name=''):
        ...
        if self.name == 'mnist' or self.name == 'synt' or self.name == 'emnist' or self.name == 'emnist26':
