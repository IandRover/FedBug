...
def __SetGrad(args, tensor, sign):
    ...
    elif args.model_name in ["resnet18", "resnet34", "mobilenetv2"]:
...
def train_model(args, model, trn_x, trn_y, dataset_name):

    n_trn = trn_x.shape[0]
    trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=args.bs, shuffle=True)
    loss_fn = torch.nn.CrossEntropyLoss(reduction='sum')
...
def train_feddecorr_model(args, model, trn_x, trn_y, dataset_name):
...
def train_feddyn_mdl(args, model, model_func, alpha_coef, avg_mdl_param, local_grad_vector, trn_x, trn_y, dataset_name):
    n_trn = trn_x.shape[0]
    trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=args.bs, shuffle=True)
    loss_fn = torch.nn.CrossEntropyLoss(reduction='sum')
...
###
def train_fedprox_mdl(args, model, avg_model_param_, mu, trn_x, trn_y, dataset_name):
    n_trn = trn_x.shape[0]
    trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=args.bs, shuffle=True)
    loss_fn = torch.nn.CrossEntropyLoss(reduction='sum')
...
def train_scaffold_mdl(args, model, model_func, state_params_diff, trn_x, trn_y, learning_rate, batch_size, n_minibatch, dataset_name):
    n_trn = trn_x.shape[0]
    trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=args.bs, shuffle=True)
    loss_fn = torch.nn.CrossEntropyLoss(reduction='sum')
